...
class Board(object):
    ...
    def is_valid_path(self, path):
        ...
        if len(path) != len(set(path)):
            ...
            for i in range(len(path) - 1):
                next_step = path[i+1]
                if next_step not in risk.definitions.territory_neighbors[path[i]]:
                    return False
        return True
    ...
    def is_valid_attack_path(self, path):
        ...
        if len(path) < 2:
            return False
        p_id = self.owner(path[0])
        for i in range(1, len(path)):
            if self.owner(path[i]) == p_id:
                return False
        return True
    ...
    def cost_of_attack_path(self, path):
        ...
        for i in range(1, len(path)):
            if self.owner(path[i]) != p_id:
                enemy_armies += self.armies(path[i])
        return enemy_armies
    ...
    def shortest_path(self, source, target):
        ...
        while q:
            current = q.popleft()
            if current == target:
                return p_dict[current]
            for territory in list(risk.definitions.territory_neighbors[current]):
                if territory not in visited:
                    copied = copy.deepcopy(p_dict[current])
                    copied.append(territory)
                    if territory in p_dict:
                        if len(copied) < len(p_dict[territory]):
                            p_dict[territory] = copied
                            q.append(territory)
                    else:
                        p_dict[territory] = copied
                        q.append(territory)
    ...
    def can_fortify(self, source, target):
        ...
        while q:
            ...
            if current == target:
                return True
            for territory in list(risk.definitions.territory_neighbors[current]):
                if self.owner(territory) == self.owner(current):
                    if territory not in visited:
                        copied = copy.deepcopy(p_dict[current])
                        copied.append(territory)
                        if territory in p_dict:
    ...
    def cheapest_attack_path(self, source, target):
        ...
        while p:
            current,cp = p.popitem()
            if current == target:
                return p_dict[current]
            for territory in risk.definitions.territory_neighbors[current]:
                if self.owner(territory) != self.owner(source):
                    if territory not in visited:
                        copied = copy.deepcopy(p_dict[current])
                        copied.append(territory)
                        pp = cp + self.armies(territory)
                        if territory not in p:
                            p_dict[territory] = copied
                            p[territory] = (pp)
                        else:
                            if pp < p[territory]:
    ...
    def can_attack(self, source, target):
        ...
        '''
        cheapest = self.cheapest_attack_path(source, target)
        return cheapest
